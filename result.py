...
Tasklist = [task_1,task_2,task_3,task_4,task_5,task_6,task_7,task_8,task_9]
...
testVec = list(processing(Tasklist))
#print(testVec)
df = pd.read_csv('paperFrame2.csv')



df['body_text'] = df['body_text'].str.slice(0, 100, 1) 
...
for item in testVec:
    ...
    index = np.array(model.docvecs.most_similar(positive=[item],topn=5),dtype=int)[:,0]
    index = list(map(int, index))
    dftemp = df.loc[index,['paper_id','title', 'abstract','body_text','url']]
    print(dftemp)
    dftemp.to_csv('task{}.csv'.format(i), encoding='utf-8', index=False)
    i += 1
